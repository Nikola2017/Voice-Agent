...
import { LANGUAGES, type AudioSource } from '@/types';
      ...
      {activeTab === 'general' && (
        <div className="space-y-4">
          ...
          <SettingRow
            icon={<Mic className="w-5 h-5" />}
            title="Whisper Mode"
            description="Use OpenAI Whisper for high-quality transcription"
            action={
              <ToggleSwitch
                enabled={recordingSettings.useWhisper}
                onToggle={() => setRecordingSettings({ useWhisper: !recordingSettings.useWhisper })}
              />
            }
          />

          {recordingSettings.useWhisper && (
            <div className="ml-12 p-3 bg-purple-500/10 rounded-lg border border-purple-500/20 space-y-3">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-white">Audio Source</p>
                  <p className="text-xs text-zinc-500">Choose audio input for recording</p>
                </div>
                <select
                  value={recordingSettings.audioSource}
                  onChange={(e) => setRecordingSettings({ audioSource: e.target.value as AudioSource })}
                  className="bg-[#241b2f] border border-purple-500/20 rounded-lg px-3 py-2 text-sm text-white"
                >
              ...
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-white">Auto-Translate</p>
                  <p className="text-xs text-zinc-500">Translate Whisper segments after recording</p>
                </div>
                <ToggleSwitch
                  enabled={recordingSettings.enableLiveTranslation}
                  onToggle={() => setRecordingSettings({
              ...
              {recordingSettings.enableLiveTranslation && (
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm text-white">Target Language</p>
                    <p className="text-xs text-zinc-500">Translate to this language</p>
                  </div>
                  <select
                    value={recordingSettings.targetTranslationLanguage}
                    onChange={(e) => setRecordingSettings({
                      targetTranslationLanguage: e.target.value as any
                    })}
                    className="bg-[#241b2f] border border-purple-500/20 rounded-lg px-3 py-2 text-sm text-white"
                  >
                    {LANGUAGES.map((lang) => (
              ...
              <p className="text-[10px] text-zinc-500">
                System audio capture requires selecting a browser tab or window with audio sharing enabled.
              </p>
            </div>
          )}
