...
import type { Note, WhisperSegment } from '@/types';
import { SPEAKER_COLORS, LANGUAGES } from '@/types';
...
export function NoteCard({ note }: NoteCardProps) {
  ...
  const [showWhisperSegments, setShowWhisperSegments] = useState(false);
  const [isRetranslating, setIsRetranslating] = useState(false);
  const [retranslateTargetLang, setRetranslateTargetLang] = useState('en');

  const isEditing = isEditingTranscript && editingNoteId === note.id;

  // Retranslate Whisper segments
  const handleRetranslate = async () => {
    if (!note.whisperSegments || note.whisperSegments.length === 0) return;
    ...
    try {
      const response = await fetch('/api/translate', {
        ...
        body: JSON.stringify({
          segments: note.whisperSegments,
          targetLanguage: retranslateTargetLang,
      ...
      if (response.ok) {
        ...
        if (data.success) {
          updateNote(note.id, { whisperSegments: data.segments });
        }
      }
    } catch (error) {
  ...
  return (
    <article className={`bg-[#0f0a15] rounded-xl border ${note.isImportant ? 'border-yellow-500/30' : 'border-purple-500/10'}`}>
      ...
      {!isMinimized && (
        ...
        {/* Whisper Segments with Translations */}
        {note.whisperSegments && note.whisperSegments.length > 0 && (
          <div className="mb-4 p-4 rounded-lg bg-[#1a1325] border border-purple-500/10">
            <div className="flex items-center justify-between mb-3">
              <button
                onClick={() => setShowWhisperSegments(!showWhisperSegments)}
                className="flex items-center gap-2 text-sm font-semibold text-white hover:text-purple-300"
              >
                <Languages className="w-4 h-4 text-blue-400" />
                Whisper Segments ({note.whisperSegments.length})
                <ChevronDown className={`w-4 h-4 transition-transform ${showWhisperSegments ? '' : '-rotate-90'}`} />
              </button>

              {/* Retranslate Controls */}
              <div className="flex items-center gap-2">
                <select
                  value={retranslateTargetLang}
                  onChange={(e) => setRetranslateTargetLang(e.target.value)}
                  className="px-2 py-1 text-xs rounded bg-[#241b2f] border border-purple-500/20 text-white"
                >
                  {LANGUAGES.map((lang) => (
                ...
                <button
                  ...
                  className="flex items-center gap-1 px-2 py-1 text-xs rounded bg-blue-500/20 text-blue-300 hover:bg-blue-500/30 disabled:opacity-50"
                >
                  {isRetranslating ? (
            ...
            {showWhisperSegments && (
              <div className="space-y-2 max-h-60 overflow-y-auto">
                {note.whisperSegments.map((segment, index) => (
                  <div key={segment.id || index} className="p-2 rounded bg-[#241b2f] text-xs">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {segment.translatedText && segment.targetLanguage && (
                        <span className="px-1.5 py-0.5 rounded bg-blue-500/20 text-blue-400 text-[10px]">
                          {LANGUAGES.find(l => l.code === segment.targetLanguage)?.flag || segment.targetLanguage}
                        </span>
                      )}
                    </div>
                    <p className="text-zinc-300">{segment.text}</p>
                    {segment.translatedText && segment.translatedText !== segment.text && (
                      <p className="text-blue-300 mt-1 pt-1 border-t border-blue-500/20">
